...
router.post('/', multer({
    //设置文件存储路径
    dest: 'uploadblogimg'   //upload文件如果不存在则会自己创建一个。
    ...
    checkToken(token, (bool) => {
        if (!bool) {
            ...
            if (req.file.length === 0) {  //判断一下文件是否存在，也可以在前端代码中进行判断。
                ...
                const fileInfo = {};
                const filemd = md5(`${token}_${date}_${originalname}`)
                const httpurlAvatar = `${url}${filemd}.${suffix}`
                
                fs.renameSync('./uploadblogimg/' + file.filename, './uploadblogimg/' + `${filemd}.${suffix}`);//这里修改文件名字，比较随意。
                // 获取文件信息
